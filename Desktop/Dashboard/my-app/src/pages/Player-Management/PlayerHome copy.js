...
const PlayerHome = ({ isOnline, otherMachineStatus }) => {
  ...
  const confirmTimeFrameChange = async (docId,activationTime,endTime) => {
    try {
    ...
    <div className="home-container">
            ...
            <div className="images">
              {storeImages.map(image  => (
                <div key={image.id} className={`image-item ${image.isActive ? '' : 'inactive'} ${selectedImage && selectedImage.docId === image.id ? 'selected' : ''}`} onClick={() => handleImageSelect(image.imageUrl, image.id)}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); openTimeFrameModal(image); }}>Set Time Frame</button>
                </div>
              ))}
            </div>
          )}
      ...
      {timeFrameModalOpen && (
        <div className="timeframe-modal">
          ...
          <button onClick={() => setTimeFrameModalOpen(false)}>Cancel</button>
        </div>
      )}
    </div>
  );
