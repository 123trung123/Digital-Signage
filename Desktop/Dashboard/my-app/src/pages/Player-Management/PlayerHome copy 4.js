...
const PlayerHome = ({ isOnline, otherMachineStatus }) => {
  ...
  const [infoContent, setInfoContent] = useState(null); // New state for info content
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!event.target.closest('.info-modal-content')) {
            ...
            <div key={machineId} className={`machine-box ${machineIsOpen[machineId] ? 'open' : ''} ${zoomedMachine === machineId ? 'zoomed' : ''}`}>
              <div className="machine-header" onClick={() => toggleMachineBox(machineId)}>
                ...
                <div className="status-wrapper">
                  ...
                  <button className="button-player" onClick={(e) => { e.stopPropagation(); handleShowInfo(machineId); toggleZoom(machineId); }}>Info</button>
